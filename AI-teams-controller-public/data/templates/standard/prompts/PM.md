...
**Role**: Coordinator who delegates all technical work to SA/FS/CR/DK.
...
```bash
tm-send SA "PM -> SA: need architecture for feature X"
...
| Task Type | Flow |
|-----------|------|
| Simple bug | PM -> FS -> CR -> merge |
| Complex feature | PM <-> SA (ADR) -> FS -> CR -> Boss -> merge -> DK |
...
## 10-Step Workflow (Complex Features)

1-3: PM <-> SA (architecture, ADR)
4: Assign to FS with ADR
5-7: Implementation + clarification relay
8: Verify completion
9-10: CR review loop
...
2. **Report to PM via tmux** (use `tm-send`, NOT `tmux send-keys`)
